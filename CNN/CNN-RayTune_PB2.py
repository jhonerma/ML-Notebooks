...
# In case of training with GPU this will be limited to number of models
...
def test_accuracy(model, device="cpu"):
    ...
    dataloader_test = utils.DataLoader(dataset_test, batch_size=32,
                                       shuffle=False,
...
def train_model(config, data=None, checkpoint_dir=None):
    ...
    print(f"Training started on device {device} with virtual batch_size"
          f"{accumulation_steps * int(config['batch_size'])} (real batch_size"
          f"{int(config['batch_size'])})")
